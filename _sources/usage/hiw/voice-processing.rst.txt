...
In the Alan infrastructure, all voice processing is performed in the Alan cloud. The Alan cloud is the AI-backend of the Alan AI Platform.
...
The Alan’s main goal is to match what the user says to a specific voice command in the script. To do this, Alan needs to rearrange unstructured data in the user input so that it can be analyzed and processed at the machine level. Every phrase said by the user passes through several steps:

1. Alan uses the Automatic Speech Recognition (ASR) engine and Speech-to-Text (STT) to get the user input and convert it to text segments.

2. With the help of Spoken Language Understanding (SLU) and Named Entity Recognition (NER) systems, Alan evaluates the phrase patterns, draws the intent and meaningful words, such as location, date and time, from the phrase. For high accuracy of matching, Alan uses the `Domain Language Model <#alan-asr-and-domain-language-model>`__ for your app.

3. Alan matches the phrase to a voice command in the Alan script. This is where Alan AI Machine Learning (ML) algorithms are leveraged. Each phrase is given a probability score, with ‘1’ being the most accurate match.

4. If Alan is supposed to give a response, it uses the Text-to-Speech (TTS) technology to synthesize speech that sounds natural.

.. tip::

   You can check the phrase probability score in Alan Studio logs. To do this, at the bottom of Alan Studio, expand the logs section and click the phrase given by the user.
...
`Alan technologies FAQ </usage/additional/faq#alan-technologies>`__
