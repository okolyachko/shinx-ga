...
When defining possible slot values, you can use the same rules as with `patterns <patterns.html>`__. You can:

-  List `multiple alternatives <patterns.html#patterns-with-alternatives>`__ separated with the \| delimiter.
-  Make slots `optional <patterns.html#optional-alternatives>`__ by adding another \| delimiter at the end of the slot.
...
You can use slots in `patterns <patterns.html>`__ for response functions: `play  <api/api-reference.html#play>`__ and
`reply <api/api-reference.html#reply>`__. Use the standard JavaScript syntax for template strings, that is, offset in backticks.
...
You can use regular expressions in `patterns <patterns.html>`__. To do this, you need to add ``*`` character to the slot name. This will signal Alan that this slot contains RegEx.
...
You can use regular expressions of any complexity. In the example below, the `intent <api/api-reference.html#intent>`__ will match all combinations of six letters and any number of digits going after the ``incident`` word. This will help if you need your users to input some uncommon abbreviations.
...
-  ``moment``: date object from the `moment.js <built-in-javascript-libraries.html#moment-timezone>`__ library 
-  ``luxon``: the `luxon </built-in-javascript-libraries.html#luxon>`__ date object
