...
1. Add another pattern to invoke the voice command. Inside this pattern, we will use the ``$(NUMBER)`` `predefined slot <../../server-api/slots.html#number>`__ to capture the number of ordered items. The command patterns are separated with a comma.
...
2. At present, our voice assistant won't understand the correct grammar form: ``Add five burritos``. To fix this, we have two options: 
   ...
   - We can use Alan's pluralizer syntax — add an underscore to the item names: ``burrito_``.
