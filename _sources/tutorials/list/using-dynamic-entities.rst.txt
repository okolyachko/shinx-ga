...
To create such 'customizable' voice commands in Alan, you can use `dynamic entities <../../server-api/dynamic-entities.html>`__. Voice commands with dynamic entities are adjusted to the data passed to them, which makes the voice assistant very adaptable.
...
-  You have `created a project <../../usage/guides/projects.html>`__ in Alan
   Studio.
...
We will use the `onCreateProject <../../server-api/api/api-reference.html#oncreateproject>`__ callback to initialize data when the project model is created.
...
2. We are joining the obtained data to receive a string like this: ``Project Power~605116|Tesla~517412|...``. The result is saved to
   ...
   `labels <../../server-api/slots.html#slot-values-and-labels>`__.
...
1. In the intent, we have: ``$(MOVIE p:videos)``. This way, we are addressing ``project.videos``, which contains the list of movies just
   as if it were a `user-defined slot <../../server-api/slots.html#user-defined-slots>`__. When the user asks for details of any movie added to ``project.videos``, Alan now plays: ``Here is something about <movieTitle>``.
...
`Dynamic entities <../../server-api/dynamic-entities.html>`__
