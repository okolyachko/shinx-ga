...
-  How to use `visual state <../../server-api/sending-data/visual-state.html>`__ to send custom data to the voice script
...
In our voice command, we will name items available in the list. For this reason, we need to send information about these items to the Alan script. To do this, we will use Alan's `visual state <../../server-api/sending-data/visual-state.html>`__.
...
`setVisualState <../../client-api/methods/common-api.html#setvisualstate>`__ method of the Alan button. On the voice script side, we will access the items array by addressing the ``p.visual.items`` variable.
...
In the IDE, open the ``MainActivity.kt`` file and in the ``MainActivity`` class, inside the ``onCreate`` function, register the `Alan callback <../../client-api/android/android-api.html#handle-events-from-the-alan-android-sdk>`__ to handle events received from the Alan voice script:
