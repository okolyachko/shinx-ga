...
You can create a voice assistant or chatbot and embed it to your React app with Alan's `voice assistant SDK for
Web <../../client-api/web/web-api.html>`__. In this tutorial, we will create a simple voice enabled React app. The app users will be able to click the Alan button and give custom voice commands, and Alan will reply to them.
...
3. We will use the Effect Hook to add the Alan button to our app. At the top of the file, add the following import statement:
   ...
   .. note:: 
      For an app that uses React Class Components, you can add the Alan button with ``componentDidMount()``. For details, see `React <../../client-api/web/react.html>`__.
...
Let's add some `voice commands <../../server-api/commands-and-responses.html>`__  so that we can interact with our React app through voice. In Alan Studio, open the project and, in the code editor, add the following intents:
