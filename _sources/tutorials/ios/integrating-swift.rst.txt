...
You can create a voice assistant or chatbot like Siri and embed it to your iOS Swift app with Alan's `voice assistant SDK for iOS <../../client-api/ios/ios-api.html>`__. In this tutorial, we will create a single view iOS app with Alan voice and test drive it on a simulator.
...
- with `CocoaPods <../../client-api/ios/ios-api.html#set-up-an-xcode-project-with-cocoapods>`__
- `manually <../../client-api/ios/ios-api.html#set-up-an-xcode-project-manually>`__
...
3. On the computer, extract ``AlanSDK.framework`` from the ZIP archive. 
4. Drag ``AlanSDK.framework`` and drop it under the root node of the Xcode project.

   .. image:: /_static/assets/img/tutorial-ios/alansdk-drag.png
...
1. We need to make sure the Alan iOS SDK is embedded when the app gets built. On the **General** tab, scroll down to the **Frameworks, Libraries, and Embedded Content** section. To the right of ``AlanSDK.framework``, select **Embed and Sign**.

   .. image:: /_static/assets/img/tutorial-ios/embed-sign.png
...
5. In Xcode, go to the **Build Phases** tab. In the top left corner, click the **+** button and select **New Run Script Phase**. In the added **Run Script** section, add the following:

   .. code:: shell

      sh "${BUILT_PRODUCTS_DIR}/${FRAMEWORKS_FOLDER_PATH}/AlanSDK.framework/frameworks-strip.sh"

   This script removes unnecessary architecture slices, which reduces
   the final package size.
   
   .. image:: /_static/assets/img/tutorial-ios/xcode-script.png
...
-  Have a look at the next tutorial: `Navigating between views in an iOS Swift app <navigating-swift.html>`__.
