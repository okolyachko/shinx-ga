...
1. `Create an Apache Cordova
   app <#step-1-create-an-apache-cordova-app>`__
...
2. (For iOS) In iOS, the user must explicitly grant permission for an app to access the device microphone and camera. The
   ``Info.plist file`` of the app must contain ``NSMicrophoneUsageDescription`` and ``NSCameraUsageDescription`` key
   with string values explaining how the app uses this data.
