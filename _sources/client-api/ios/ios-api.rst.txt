...
7. In iOS, the user must explicitly grant permission for an app to access the microphone. The ``Info.plist`` file of the app must contain the ``NSMicrophoneUsageDescription`` key with a string value explaining to the user how the app uses this data. To add the key:
